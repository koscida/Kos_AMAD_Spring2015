...
func createLabelTitleLeftJustified(name name: String, text: String, width: CGFloat, height: CGFloat, x: CGFloat, y: CGFloat, textColor: UIColor) -> SKLabelNode {
    let node = createLabelFillRect(name: name, text: text, width: width, height: height, x: x, y: y, textColor: textColor)
	node.verticalAlignmentMode = SKLabelVerticalAlignmentMode.Top
...
func createSimpleButtonCentered(name name: String, text: String, x: CGFloat, y: CGFloat, widthExact: CGFloat, heightExact: CGFloat, textColor: UIColor, fillColor: UIColor) -> SKNode {
    ...
    let label = createLabelFillRect(name: name, text: text, width: widthExact - (popupPadding*2), height: heightExact - (2*popupPadding), x: x, y: y, textColor: textColor)
    btn.addChild(label)
...
func createSceneStart() -> SKNode {
	...
	let titleLabel3 = createLabelTitleCentered(name: "", text: "at 77%", x: widthMid, y: heightMid - 100, textColor: colorStartText)
	node.addChild(titleLabel3)
	
	let playButton = createPopupButtonLarge(name: nextSceneButtonName, text: "PLAY", x: widthMid, y: heightMid - 300)
...
func createSceneEnd() -> SKNode {
	let node = SKNode()
	node.name = ""
	
	let titleLabel1 = createLabelTitleCentered(name: "", text: "End", x: widthMid, y: heightMid + 300, textColor: colorStartText)
	node.addChild(titleLabel1)
	
	let playButton = createPopupButtonLarge(name: nextSceneButtonName, text: "Play Again", x: widthMid, y: heightMid - 300)
	node.addChild(playButton)
...
func createBuyConfirmMessage(itemSelected itemSelected: Item) -> SKNode {
	...
	let msg = createLabelFillRect(name: "msg", text: itemSelected.tryMessages[itemSelected.buyAttempts-1], width: errorMessageWidth, height: popupTextRegularHeight, x: buyConfirmLabelX, y: buyConfirmLabelY, textColor: colorBlack)
	node.addChild(msg)
...
//// ///////////////////////////////////////////////////////////////////// ////
////                                                                       ////
////              Helper Functions - TRAIT ERROR            
////                                                                       ////
//// ///////////////////////////////////////////////////////////////////// ////
//// ///////////////////////////////////////////////////////////////////// ////
func createErrorTrait(traitIndex traitIndex: Int) -> SKNode {
	let node = SKNode()
	node.name = "traitError"
	node.zPosition = zPositionItemDetailPopup
	...
	// confirm
	let confirmButton = createPopupButtonLarge(name: errorTraitButtonOk, text: "OKAY", x: errorButtonX, y: errorButtonY)
	node.addChild(confirmButton)
	
	// item warning message
	node.addChild(
		createLabelFillRect(name: "", text: "Error: You do not have enough of the \"" + traitsDataOrigin[traitIndex].name + "\" trait to choose this action", width: errorMessageWidth, height: errorMessageHeight, x: errorMessageX, y: errorMessageY, textColor: colorPopupText)
	)
...
func createChangingTraits() -> SKNode {
	...
	for trait in traitsDataOrigin {
		...
		let label = createLabelFillRect(name: trait.name, text: trait.name, width: changingStatsLabelWidth, height: changingStatsLabelHeight, x: changingStatsLabelX, y: runningY, textColor: colorWhite)
		node.addChild(label)
		
		let bar = createChagingStatsBarBack(name: trait.name + "_changingStatsTraitBack", x: changingStatsTraitX, y: runningY)
		node.addChild(bar)
		
		let newLevelWidth = calcBarWidth(maxWidth: changingStatsTraitWidth, currentValue: trait.value, maxValue: trait.max)
		let level = createChangingStatsBarLevel(name: trait.name + changingStatsTraitSuffix, xLeft: changingStatsTraitXLeft, y: runningY, width: newLevelWidth)
		node.addChild(level)
...
func createTraitsStats() -> SKNode {
    ...
    var runningXLeft = statusTraitFirstXLeft
    var index = 0
    for trait in traitsDataOrigin {
...
func createInventoryMenu() -> SKNode {
    ...
    // title
    let title = createLabelTitleLeftJustified(name: "inventoryTitle", text: "Inventory", width: width/2, height: inventoryTitleHeight, x: inventoryTitleXLeft, y: inventoryTitleY, textColor: colorInventoryTitle)
    node.addChild(title)
