...
class SceneBase: SKScene {
        ...
        for touch in touches {
            ...
            if let name = touchedNode.name {
                ...
                case levelMode_ItemDetail, levelMode_ItemDetail_Message, levelMode_ItemDetail_ErrorMoney, levelMode_ItemDetail_ChangingStats :
					...
					// ok from error - not enough trait
					else if name == errorTraitButtonOk {
						runAction_ExitErrorTrait()
					}
    ...
    override func update(currentTime: CFTimeInterval) {
		...
		if levelMode == levelMode_Start {
			for n in sceneNode_content.children { // this is the container node
				...
				if ( (node.position.x + node.width - itemDisplayWidth) <= width ) 
					&& 
					(sceneNode_content.children.count == 1) {
					let node2 = createItems()
					//node2.position.x = node.position.x + node.width + itemPadding + itemDisplayXIncrament
					node2.position.x = node.position.x + node.width - itemPadding + (itemDisplayWidth/2) - itemDisplayXIncrament
					sceneNode_content.addChild(node2)
				}
				
				// check if off screen - remove it
				if ((node.position.x + node.width + itemDisplayWidth) <= 0) {
	...
	func runAction_ShowBuyMessage() {
		...
		print("end runAction_ShowBuyMessage")
	}
	...
	///////////////////////////////////
	//  item detail - error - trait  //
	///////////////////////////////////
	func runAction_ShowErrorTrait(traitIndex traitIndex: Int) {
		print("runAction_ShowErrorTrait (SceneBase) " + name!)
		
		// change mode
		levelMode = levelMode_ItemDetail_ErrorTrait
		
		// add message popup to item detail popup
		sceneNode_itemDetailPopup.addChild(createErrorTrait(traitIndex: traitIndex))
		addChild(sceneNode_itemDetailPopup)
		animatingAlpha = true; nodeToChange = sceneNode_itemDetailPopup; nodeToChange.alpha = 0
	}
	func runAction_ExitErrorTrait() {
		print("runAction_ExitErrorTrait (SceneBase) " + name! + "\n")
	...
	func runAction_UpdateMenuOverlayStats() {
		...
		for trait in traitsDataOrigin {
			...
			node.size = CGSize(width: newLevelWidth, height: node.size.height)
			node.position = CGPoint(x: runningXLeft + (newLevelWidth/2), y: node.position.y)
    ...
    func runLogic_AttemptBuyItem() {
        ...
        if(moneyTotal >= itemSelected.detailCost) {
            ...
            if itemSelected.buyAttempts >= itemSelected.buyMinAttempts {
				print("-- Have Min Tries --")
				
				// CHECK: if trait limit 
				if itemSelected.traitLimited {
					print("-- Trait Limited --")
					//check all traits
					var index = 0
					var badIndex = -1
					for minTraitValue in itemSelected.traitLimitValues {
						print("index: " + String(index) + " minTraitValue: " + String(minTraitValue) + " traitsDataOrigin[index].value: " + String(traitsDataOrigin[index].value))
						if (minTraitValue != 0) && minTraitValue > traitsDataOrigin[index].value {
							badIndex = index
						}
						index += 1
					}
					if badIndex == -1 {
						...
						// do not have enough of this trait
						runAction_ShowErrorTrait(traitIndex: badIndex)
					}
